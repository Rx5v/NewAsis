    ...
    let data = await dbuse.query(
      {
        namedPlaceholders: true,
        dateStrings: true,
        sql: `SELECT p.*, JSON_QUERY(p.userLog, '$') AS userLog, JSON_QUERY(p.jnsProses, '$') AS jnsProses, JSON_QUERY(p.detOrder, '$') AS detOrder, c.namaPartner, c.telpPIC,
...
app.post("/projectDone", async (req, res) => {
  ...
  try {
    ...
    await dbuse.batch(
      {
        namedPlaceholders: true,
        sql: `INSERT INTO detTrans (nomorBukti, kodeProduk, qty, jmlHarga, dpp, hpp, jmlKemasan, hargaKemasan, jmlPoint, kemasan)
      VALUES ('${x.nomorBukti}', :kodeProduk, :qtyInv, :jmlHarga, :jmlHarga, :hpp, :qtyInv, :jmlHarga / :qtyInv, :jmlPoint, :kemasan)`,
      },
      detInv
    );
    // update stok
...
app.post("/taskData", async (req, res) => {
  ...
  try {
    let data = await dbuse.query(
      ...
      c.alamat, c.email, cb.namaCabang, cb.alamatCabang,
        COALESCE(b.biaya, 0) AS biaya, j.namaJenis AS jenisProject, j.akunUangmuka, pp.operator AS namaSales, pp.jnsProses as pJenisProses, pp.operator, pp.statusProses, pp.nomorProses, pp.hasilBagus, pp.hasilJelek, pp.sisa, pp.tglStart,pp.tglFinish, pp.totalWaktu
      FROM project p
        LEFT JOIN partner c ON c.kodePartner = p.kodePartner
        LEFT JOIN karyawan ky ON ky.kodeKar = p.salesID
        LEFT JOIN cabang cb ON cb.kodeCab = p.kodeCab
  ...
  try {
    const data = await dbuse.query({
      ...
      sql: `UPDATE prosesProject SET sisa = '${(
        ...
        (parseFloat(hasilBagus) + parseFloat(x.good)) +
        (parseFloat(hasilJelek) + parseFloat(x.reject))
      }, mesin = '${x.mesin}', totalWaktu = '${
        x.totalWaktu +
        Math.abs(new Date(x.tglFinish) - new Date(x.tglStart)) / 60000
...
app.post("/svProses", async (req, res) => {
  ...
  try {
    ...
    if (!x.isRevisi) {
      state = "Selesai Proses"; // Set status
...
app.post("/getReportProject", async (req, res) => {
  const { x } = req.body;
  try {
    const data = await dbuse.query(
      `select pp.nomorProject,p.operator, p.nomorProses, p.hasilBagus, p.hasilJelek, p.sisa, p.mesin,JSON_QUERY(p.logProses, '$') AS logProses, p.totalWaktu from project pp left join prosesProject p on pp.nomorProject = p.nomorProject where pp.status = 'Selesai' AND (pp.tglMasuk BETWEEN '${x.tgla}' AND DATE_ADD('${x.tglb}', INTERVAL 1439 MINUTE))`
    );
    res.status(200).send(data);
  } catch (error) {
    console.log(error);
