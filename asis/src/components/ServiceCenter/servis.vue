  ...
  <q-card>
        ...
        <template v-slot:body="props">
          ...
          >
            <q-td
              ...
              style="max-width: 250px; word-wrap: normal"
            >
              ...
              <template v-else-if="col.name === 'namaTeknisi'">
                ...
                <q-popup-edit
                  ...
                  buttons
                  label-set="Simpan"
                  @save="upD(props.row, 'Ubah Teknisi')"
                >
                  <q-select
                    v-model="props.row.teknisi"
                    dense
              ...
              <template v-else-if="col.name === 'status'">
                ...
                <q-popup-edit
                  ...
                  buttons
                  label-set="Simpan"
                  @save="upD(props.row, props.row.status)"
                >
                  <q-select
                    v-model="props.row.status"
                    dense
    ...
    <q-dialog v-model="adS">
      <q-card>
        ...
        <q-card-section>
          <q-form @submit="addServis(p)" @reset="onReset" class="q-gutter-md">
            <div class="row justify-between">
              <div class="col-12 col-md-5">
                <q-select
                  ...
                  use-input
                  @filter="filterFn"
                  v-model="p.kodeProduk"
                  :options="options"
                  :option-label="(item) => item && item.namaBarang"
              ...
              <div class="col-12 col-md-5">
                ...
                <q-select
                  v-model="p.teknisi"
                  :options="dtSales.filter((s) => s.jabatan === 'Teknisi')"
                  :option-label="
                    (item) => item && item.namaKaryawan + ' ' + item.namaCabang
                  "
                  option-value="salesID"
          ...
          >
            <template v-slot:top>
              <div class="fit row wrap justify-end items-start content-start">
                ...
                <q-select
                  ...
                  :options="['Proses', 'Part', 'Konfirm', 'Selesai', 'Batal']"
                  class="q-ml-md text-bold"
                  style="width: 250px"
                  color="primary"
                  @input="upD(servis)"
                />
                <q-chip color="primary" outline class="text-white text-bold">
            ...
            <template v-slot:bottom-row>
              <q-tr>
                ...
                <q-td align="right"
                  >{{ detTrx.reduce((a, b) => a + b.jmlHarga, 0) | duit }}
                </q-td>
              </q-tr>
            </template>
          </q-table>
...
export default {
  ...
  setup(props, { root }) {
      ...
      serviceAll(dt.filt).then(({ data }) => {
        dt.dataAllServis = data.map((a) => {
          if (a.pencapaian) {
            ...
            a.totalPointTeknisi =
              a.pointService + a.pointRefill + a.pointMaintenance;
          }
          return a;
        });
      });
    ...
    const dataServis = computed(() => {
      return dt.dataAllServis.filter((a) =>
        dt.filt.status.some((s) => s === a.status)
      );
    });
    dtUser().then(({ data }) => {
      dt.dtKar = data;
    ...
    const addServis = (x) => {
      ...
      if (dt.cust.kodePartner && x.pointService) {
        ...
        root.$q.notify({
          message: "Isi data pelanggan dan cek point service...",
          color: "warning",
        });
      }
    };
      ...
      root.$q
        .dialog({
          title: `Update Nomor Servis : ${x.nomorServis} Status ${
            z || x.status
          }<br><span class="text-accent text-italic"> Telp PICs: ${
            x.telpPIC || x.tlp
          }</span>`,
          message: "",
          options: {
      ...
      root.$q
        .dialog({
          ...
          options: {
            ...
            isValid: (val) =>
              ["Closed", "Ambil", "Batal", "Tutup"].some((a) => a === val),
          },
          ok: {
            push: true,
          },
  ...
  <br/>
                      ...
                      <th align="center">Diterima Oleh,<br>Admin<br><br><br><br>(${
                        x.namaSales
                      })</th>
                      <th align="center">Diserahkan Oleh,<br>Pelanggan<br><br><br><br>( &nbsp; ${
                        x.namaPIC
                      } &nbsp; )</th>
                    </tr>
                  </tbody></table>
    ...
    const printW = async (x) => {
      if (x.status === "Batal") {
        ...
        let detail = await partService(x);
                  ...
                  <td class="bdr_btm" align='right'>
                      <table width="100%" >
                          ...
                          <tr>
                              ...
                              <td>: <b>${
                                x.nomorBukti || "Belum ada tagihan"
                              }</b></td>
                          </tr>
                          <tr>
                              <td>Tanggal </td>
    ...
    const printWr = async (x) => {
      let cabA = dt.cabAll.find((a) => a.kodeCab === x.kodeCab);
      let detail = dataServis.value.filter((a) =>
        dt.pilih.some(
          (s) =>
            s.nomorServis === a.nomorServis && a.kodePartner === x.kodePartner
        )
      );
      let wd = window.open("", "transPrint", "resize = 1");
      let html = `<html><head>`;
    ...
    const toDown = (data, z) => {
      let cabang = dt.filt.kodeCab.map(
        (a) => dt.cabAll.find((s) => s.kodeCab === a).namaCabang
      );
