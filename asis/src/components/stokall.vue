...
import { company, dtCab, getProdukCat, stokall } from '../services/apiList'
...
export default {
  setup (props, { root }) {
    ...
    const jdl = computed(() => {
      ...
      for (let i in dt.allCabang) {
        j.push({ name: dt.allCabang[i].kodeCab, label: dt.allCabang[i].namaCabang, field: row => row[dt.allCabang[i].kodeCab], jml: 'Y', align: 'right', sortable: true })
      }
      return j
    })
    ...
    return { ...toRefs(dt), jdl, getStok, pilihan, gantiCab, filterFn, pilKat, dtBarang }
  },
  watch: {
    selected: function (v) {
