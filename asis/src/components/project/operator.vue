            ...
            >
              <template v-if="col.name === 'act'">
                <q-btn
                  ...
                  outline
                  v-if="
                    props.row.operator == $store.state.auth.user.nama 
                    && !['proses press', 'proses print', 'proses cuttting', 'Disimpan'].includes(props.row.status)
          
                  "
                  @click="inProj('Proses', props.row)"
                >
                  <q-tooltip
                    content-class="bg-blue-9"
                    :offset="[10, 10]"
                    anchor="center left"
                  >
                    Isi data
                ...
                <q-btn
                  ...
                  v-if="
                    props.row.operator == $store.state.auth.user.nama &&
                    (props.row.status == 'proses press' ||
                      props.row.status == 'proses print' ||
                      props.row.status == 'proses cuttting')
                  "
                  ...
                  <template
                    v-if="
                      [
                        ...
                        'Revisi',
                        'Disimpan',
                        'selesai proses print',
                        'selesai proses press',
                        'selesai proses cutting',
                      ].some((a) => props.row.status === a)
                    "
                  >
                    <q-btn
                      icon="play_circle"
...
import inputProject from "./detailProject.vue";
  ...
  setup(props, { root }) {
    const dt = reactive({
      ...
      filt: {
        ...
        tgla: "",
        tglb: "",
        op: root.$store.state.auth.user.nama,
    ...
    const dataProject = computed(() => {
      return dt.dataAllProject.filter((a) =>
        [
          "Approve",
          "Proses",
          "Disimpan",
      ...
      ); /* .filter(a => dt.filt.status.some(s => s === a.status)) */
    });
    const kerjakan = (x) => {
      console.log(x);
      let jenisProses = JSON.parse(x.jnsProses);
      let processed = JSON.parse(x.processed);
      let option = [];
      jenisProses.map((a) => {
        if (processed !== null) {
          processed.some((s) => s == a)
            ? option.push({
                label: a + " (sudah dikerjakan)",
                value: a,
                color: "deep-purple-4",
                disable: true,
              })
...
<html>
  ...
  <body>
    ...
    const stop = (x) => {
      root.$q
        .dialog({
          title: "Hentikan sekarang",
          message: "Pastikan anda sudah benar benar selesai..",
          cancel: true,
          color: "teal",
          persistent: true,
        })
