<template>
  <q-card style="min-width: 700px">
    <q-card-section>
      ...
      >
        <template v-slot:top>
          <div class="col-12 q-table__title text-h6 text-blue">
            Rekap Project
          </div>
          <q-chip color="blue-6" class="text-white text-bold q-ml-md"
            ...
            >
              <q-date
                range
                v-model="tgl"
                @input="(x) => x && (rkp(), $refs.qDateProxyb.hide())"
                mask="YYYY-MM-DD"
                lazy-rules
              />
            </q-popup-proxy>
        ...
        <template v-slot:body="props">
          ...
          <q-tr v-show="props.expand" :props="props">
            <q-td colspan="100%">
              <template
                v-if="
                  detTrx.some((a) => a.nomorProject === props.row.nomorProject)
                "
              >
                <q-markup-table>
                  <q-tr>
                  ...
                  >
                    ...
                    <q-td align="right">{{ dt.qtySPK }} </q-td>
                    <q-td align="right">{{ dt.qtyProd }} </q-td>
                    <q-td align="right">{{
                      (dt.hargaSat * dt.qtySPK) | duit
                    }}</q-td>
                  </q-tr>
                </q-markup-table>
              </template>
              <template v-else>
                <span style="font-style: italic; font-color: orange"
                  >Belum ada detail {{ props.row.nomorProject }}</span
                >
              </template>
            </q-td>
          </q-tr>
