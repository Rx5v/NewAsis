    ...
    <div class="q-pa-xs row q-gutter-xs justify-end">
      <div class="text-h6">
        ...
        <q-chip color="blue" class="text-white"
          >{{ tgl !== null && tgl.from ? tgl.from + " ~ " + tgl.to : tgl }}
          <q-popup-proxy
            ref="qDateProxy"
      ...
      >
        ...
        <template v-slot:selected> {{ filt.kodeCab.length }} cabang </template>
      </q-select>
      <q-toggle
        v-model="filt.allCab"
        label="Pilih semua..."
    ...
    <div class="row q-gutter-sm justify-around">
      ...
      <div class="q-pa-xs col-xs-12">
        <q-card>
          ...
          <q-list dense>
            <q-item
              v-for="col in jdl.filter(col => col.name !== 'namaCabang')"
              :key="col.name"
            >
              <q-item-section>
                ...
                <q-item-label caption>
                  <template v-if="col.jml === 'Y'">
                    <q-chip outline color="primary">{{
                      dtSales.reduce((a, b) => a + b[col.name], 0) | nomer
                    }}</q-chip>
                  </template>
          ...
          <template v-slot:item="props">
            ...
            >
              <q-card>
                <q-card-section
                  :style="
                    `background: linear-gradient(to left, transparent,rgba(${200 -
                      (props.row.jmlHarga / total.jmlHarga) * 225},${(props.row
                      .jmlHarga /
                      total.jmlHarga) *
                      255},${(props.row.jmlHarga / total.jmlHarga) * 125 +
                      200},0.8))`
                  "
                >
                  <q-chip class="text-white bg-blue">{{
                ...
                <q-list dense>
                  ...
                  >
                    ...
                    <q-item-section side>
                      <q-item-label caption>
                        <template
                          v-if="col.name === 'hpp' || col.name === 'jmlHargaR'"
                        >
      ...
      <div
        class="q-pa-xs col-xs-12 col-sm-5 col-md-5 col-lg-3"
        v-if="datasetk.length"
      >
        <donut
          :labels="labelk"
          :datasets="datasetk"
      ...
      <div
        class="q-pa-xs col-xs-12 col-sm-5 col-md-5 col-lg-3"
        v-if="datasett.length"
      >
        <garis
          :labels="labelt"
          :datasets="datasett"
          type="area"
          :title="{ text: 'Omzet per Tanggal' }"
          class="bg-teal-13"
        ></garis>
      </div>
        ...
        >
          ...
          <template v-slot:header>
            <q-tr class="text-bold bg-info">
              ...
              <q-th
                colSpan="2"
                v-for="(a, s) in filt.kodeCab"
                :key="a"
                :class="s % 2 === 0 && 'bg-teal-13'"
              >
                {{
                  cabAll.find(c => c.kodeCab === a) &&
                    cabAll.find(c => c.kodeCab === a).namaCabang
                }}
              </q-th>
            </q-tr>
            <q-tr class="text-bold bg-info">
          ...
          <template v-slot:bottom-row>
            <q-tr class="text-bold">
              <template v-if="rkpTgl.length">
                <q-th
                  v-for="(s, a, i) in rkpTgl[0]"
                  :key="i"
                  :class="i % 2 === 0 && 'bg-teal-13'"
                >
                  <template v-if="i === 0">
                    Total
                  </template>
...
export default {
  ...
  setup (props, { root, refs }) {
    ...
    const perProfit = computed(() => {
      const fl = dt.jnsTop === 'Omzet' ? 'jmlHarga' : 'profit'
      const a = dt.perKate.sort(function (a, b) {
        return b[fl] - a.[fl]
      })
      return a
    })
    ...
    return { ...toRefs(dt), toDown, jdlRkp, lihat, rkpJam, total, gantiCab, perProfit, perDiv }
  },
  watch: {
    counter: function (v) {
      // if (v % 5000 === 0) {
