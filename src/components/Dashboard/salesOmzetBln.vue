  ...
  <q-card class="fit bg-teal-12">
    ...
    <q-card-section>
      <q-table
        ...
        >
        <template v-slot:top>
          <garis :labels="labelk" :datasets="dataChart" type="line" :title="{ text: 'Sales Omzet Per Salesman' }" class="bg-cyan-13"></garis>
        </template>
  ...
  setup (props, { root }) {
    const dt = reactive({
      ...
      jdlBln: [
        ...
        { name: props.jenis.jnsGrup, label: `Nama ${props.jenis.nama}`, field: row => row[props.jenis.jnsGrup], align: 'left', sortable: true },
