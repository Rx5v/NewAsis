    ...
    <q-card class="print-hide">
      <q-card-section>
        <q-table
          ...
          dense>
          <template v-slot:top>
            <div class="col-12 q-table__title text-center text-white">Data Transaksi {{ jenis.judul }}</div>
            <div class="col-6 text-white">
              ...
              <template v-if="!perPartner">
                <q-checkbox v-model="caraBayar" val="tempo" label="tempo" color="orange" keep-color/>
                <q-checkbox v-model="caraBayar" val="tunai" label="tunai" color="teal" keep-color/>
                <q-checkbox v-model="caraBayar" val="sales" label="sales" color="teal" keep-color/>
                <q-checkbox v-model="caraBayar" val="Bank" label="Transfer" color="teal" keep-color/>
                <q-checkbox v-model="caraBayar" val="EDC" label="EDC" color="teal" keep-color/>
              </template>
            </div>
            <div class="col-6 text-white">
              <q-input dense debounce="300" v-model="cari" placeholder="Cari..." class="q-ml-md">
          ...
          <template v-slot:body="props">
            <q-tr :props="props" :style="props.expand ? 'background: linear-gradient(to bottom, #00ccff 30%, #ffffff 92%)' : props.row.status !== 'T' ? 'background: linear-gradient(to left, #f50083, #ffffff 70%)' : ''">
              <q-td auto-width>
                <q-icon v-if="perPartner" size="sm" color="accent" dense @click="props.expand = !props.expand" :name="props.expand ? 'expand_less' : 'expand_more'" />
                <q-icon v-else size="sm" color="accent" dense @click="getdetTrx(props.row, props.expand),props.expand = !props.expand" :name="props.expand ? 'expand_less' : 'expand_more'" />
                {{ props.rowIndex + 1}}
              </q-td>
              <q-td
                v-for="col in props.cols"
              ...
              <q-td auto-width v-if="!perPartner" :class="(props.row.jurnalReff === null && props.row.ancab === 'Y' && props.row.status === 'T') && 'bg-orange-5'">
                ...
                <q-btn icon="fas fa-balance-scale" :color="(props.row.nomorJurnal.length || props.row.jurnalReff) ? 'teal' : 'pink'" dense outline class="q-ml-xs"
                ...
                <q-btn @click="repairJurnalServis(props.row)" icon="fas fa-cogs" dense outline class="q-ml-xs text-purple-12" v-if="$store.state.auth.user.akun === 'itsupport'">
                  <q-tooltip content-class="bg-brown-3 text-bold" :offset="[10, 10]" anchor="bottom left">
                    Repair Jurser
                  </q-tooltip>
                </q-btn>
                <q-btn label="Bukti" color="cyan-5" @click="getSPK(props.row)" v-if="props.row.spkFile"/>

                <q-btn small outline round class="q-ml-sm" icon="attach_file" @click="getLamp(props.row)"/>
            ...
            <q-tr v-show="props.expand" :props="props">
              <template v-if="perPartner">
                <q-td colspan="100%">
                  <q-table
                    class="detTrans"
                    :data="dataTrx.filter(a => a.kodePartner === props.row.kodePartner)"
                    :columns="jdl"
                    row-key="nomorBukti"
                    :filter="cari"
                    dense>
                    <template v-slot:body="pp">
                      <q-tr :props="pp" :class="props.expand && 'bg-yellow-2'">
                        ...
                        >
                          <template v-if="col.name === 'kodeCab'">
                            <q-icon size="sm" color="accent" dense @click="getdetTrx(pp.row, pp.expand),pp.expand = !pp.expand" :name="pp.expand ? 'expand_less' : 'expand_more'" />
                            {{ jnsTrx === 'J' ? pp.row.kodeCab : pp.row.tujuan }}
                          </template>
                          <template v-else-if="col.name === 'tglKirim'">
                            {{ col.value }}
                            <q-icon v-if="['MAN','purchase', 'mitra', 'acc'].some(a=> a== $store.state.auth.user.userType)" name="edit" class="q-ml-xs" color="red" @click="ubahTgl(pp.row)" />
                          </template>
                          <template v-else-if="col.name === 'namaSales'">
                            {{ col.value }}
                            <q-popup-edit v-model="props.row.gantiSales" buttons @save="ubahSales(props.row)">
                              <q-select
                                ...
                                use-input
                                :options="pilihSales.filter(a => a.kodeCab === pp.row.cabID)"
                                :option-label="(item) => item && item.namaKaryawan +' '+ item.namaCabang"
                                option-value="salesID"
                                options-dense
                                map-options
                        ...
                        <q-td auto-width :class="(pp.row.jurnalReff === null && pp.row.ancab === 'Y' && pp.row.status === 'T') && 'bg-orange-5'">
                          <q-btn icon="fas fa-edit" color="red-13" dense outline class="q-ml-xs" v-if="['MAN','purchase', 'mitra', 'acc'].some(a=> a== $store.state.auth.user.userType) && pp.row.status !== 'W'"
                            @click="buka(pp.row)">
                            <q-tooltip content-class="bg-orange-13" :offset="[10, 10]" anchor="center left">
                              Buka transaksi
                            </q-tooltip>
                          </q-btn>
                          <q-btn v-if="pp.row.status === 'W'" icon-right="edit" color="orange" fab-mini outline @click="editJubel(pp.row)">
                            <q-tooltip content-class="bg-orange" :offset="[10, 10]" anchor="center left">
                              Edit
                            </q-tooltip>
                          </q-btn>
                          <q-btn v-if="['W', 'D'].some(a => a === pp.row.status)" icon-right="check" color="teal" fab-mini outline
                            @click="pr=pp.row,tompo(pp.row)" class="q-ml-xs">
                            <q-tooltip content-class="bg-cyan" :offset="[10, 10]" anchor="center left">
                              Konfirm
                            </q-tooltip>
                          </q-btn><!--
                          ...
                          <q-btn v-if="pp.row.status === 'T' && pp.row.garansi==='Y' && ['MAN','purchase', 'admin', 'mitra', 'acc'].some(a=> a== $store.state.auth.user.userType)"
                            icon-right="replay"
                            color="warning" flat @click="returBarang(pp.row)" class="q-ml-xs">
                            <q-tooltip content-class="bg-orange" :offset="[10, 10]" anchor="center left">
                              Retur
                            </q-tooltip>
                          </q-btn>
                          <q-btn icon="print" dense outline color="orange" @click="getdetTrx(pp.row, pp.expand), cetak(pp.row)" class="q-ml-xs"/>
                          <q-btn icon="fas fa-balance-scale" color="teal" dense outline class="q-ml-xs"
                            v-if="['MAN','purchase', 'admin', 'mitra', 'acc'].some(a=> a== $store.state.auth.user.userType) && ['T', 'D'].some(a => a === pp.row.status)"
                            @click="cj=true,getJur(pp.row)">
                            <q-tooltip content-class="bg-teal" :offset="[10, 10]" anchor="center left">
                              Jurnal
                            </q-tooltip>
                          </q-btn>
                          <q-btn icon="fas fa-money-bill-alt" color="blue-13" dense outline class="q-ml-xs" v-if="['MAN','purchase', 'admin', 'mitra', 'acc'].some(a=> a== $store.state.auth.user.userType) && pp.row.status === 'T' && pp.row.walkIn === 'Y'">
                            <q-tooltip content-class="bg-blue-13" :offset="[10, 10]" anchor="center left">
                              Klaim Point
                            </q-tooltip>
                          </q-btn>
                          <q-btn @click="updPoint(pp.row)" icon="fas fa-money-bill-alt" dense outline class="q-ml-xs text-green-12" v-if="(['MAN','purchase', 'mitra', 'acc'].some(a=> a== $store.state.auth.user.userType) && pp.row.point === 'Y' && pp.row.ac === 'N' && pp.row.cabID !== 'AM01' && pp.row.ancab !== 'Y') || pp.row.totalPoint > 0">
                            <q-tooltip content-class="bg-brown-3 text-bold" :offset="[10, 10]" anchor="bottom left">
                              Repair Point
                            </q-tooltip>
                          </q-btn>
                          <q-btn label="SPK Dinas" @click="getSPK(pp.row)" v-if="pp.row.spkFile"/>
                          <q-btn rounded icon="attach-file" @click="getLamp(pp.row)"/>
                        </q-td>
                      </q-tr>
                      <q-tr v-show="pp.expand" :props="pp">
                          <q-td colspan="100%">
                            <q-table
                              class="detTrx"
                              :data="detTrx.filter(a => a.nomorBukti === pp.row.nomorBukti)"
                              :columns="jdle"
                              row-key="kodeProduk"
                              separator="cell"
                              ...
                              <template v-slot:body="ppt">
                                <q-tr :props="props">
                                  <q-td class="text-right">
                                    {{ detTrx.filter(a => a.nomorBukti === pp.row.nomorBukti).indexOf(ppt.row) +1 }}
                                  </q-td>
                                  ...
                                  <q-td key="jmlKemasan" :props="ppt">
                                    {{ ppt.row.jmlKemasan | nomer }} {{ ppt.row.kemasan.kemasan }}
                                  </q-td>
                              ...
                              <template v-slot:bottom-row>
                                ...
                                <q-tr>
                                  ...
                                  <q-th align="right">{{ pp.row.totalAkhir | duit }}</q-th>
                                </q-tr>
                              </template>
                            </q-table>
                          </q-td>
              ...
              <template v-else>
                <q-td colspan="100%">
                  <q-table
                    class="detTrx"
                    :data="detTrx.filter(a => a.nomorBukti === props.row.nomorBukti)"
                    :columns="jdle"
                    row-key="kodeProduk"
                    separator="cell"
                    :pagination.sync="hald"
                    ...
                    <template v-slot:body="props">
                      <q-tr :props="props">
                        <q-td class="text-right">
                          {{ detTrx.filter(a => a.nomorBukti === props.row.nomorBukti).indexOf(props.row) +1 }}
                        </q-td>
                        ...
                        <q-td key="jmlKemasan" :props="props">
                          {{ props.row.jmlKemasan | nomer }} {{ props.row.kemasan.kemasan }}
                        </q-td>
          ...
          <template v-slot:bottom-row>
            <q-tr>
              <q-th :colspan="perPartner ? 3 : 6" class="text-right">Jumlah</q-th>
              <q-th align="right">{{ transaksi.reduce((a,b) => $dwn.jumlah([a, b.totalAkhir]), 0) | duit }}</q-th>
              <q-th/>
              <q-th align="right">{{ transaksi.reduce((a,b) => $dwn.jumlah([a, b.totalPoint]), 0) | duit }}</q-th>
            </q-tr>
          </template>
        </q-table>
      </q-card-section>
    ...
    <q-dialog v-model="confirm" persistent>
      <q-card>
        ...
        <q-card-actions align="right">
          ...
          <q-btn outline label="Surat Jalan SN" color="primary" @click="printSjln(ctk, true)" v-if="['MAN','purchase', 'gudang', 'mitra', 'acc'].some(a=> a== $store.state.auth.user.userType)"/>
    ...
    <q-dialog
      ...
      <q-card>
        <q-card-section>
          ...
          <q-btn icon="add" dense rounded color="pink-13" @click="addLamp = true" class="text-right q-ml-md"/>
              ...
              <q-item dense>
                ...
                <q-item-section side>
                  ...
                  <q-btn-group class="">
                    ...
                    <q-icon name="close" color="orange-13" size="md" @click="delLamp(a)" v-if="['MAN', 'purchase', 'mitra', 'acc'].includes($store.state.auth.user.userType) || pr.status === 'W'">
...
<script>
import { dtTrx, detTrxR, detTrx, konfirm, dtCab, getJurtrans, getBankCab, editTglTrx, perbaiki, carikar, gantiSales, repairPoint, cekRabalanceServis, getLog } from '../services/apiList'
...
export default {
  ...
  setup (props, { root }) {
    const dt = reactive({
      ...
      jdld: [
        ...
        { name: 'harga', label: 'Harga @', field: row => (row.hargaKemasan), jml: 'Y', align: 'right' },
      ...
      jdle: [
        ...
        { name: 'jmlKemasan', label: 'Kemasan', field: row => row.jmlKemasan, jml: 'Y', align: 'right' },
        { name: 'jmlRetur', label: 'Retur', field: row => row.jmlRetur, jml: 'Y', align: 'right' },
        { name: 'hargaKemasan', label: 'Harga @', field: row => (row.hargaKemasan), jml: 'Y', align: 'right' },
        { name: 'jmlHarga', label: 'jmlHarga', field: row => row.jmlHarga, jml: 'Y', align: 'right' },
      ...
      jdlp: [
        ...
        { name: 'totalAkhir', label: 'Total Harga', field: row => row.totalAkhir, jml: 'Y', align: 'right', sortable: true },
      ...
      perPartner: false,
      rkpTrx: [],
      filt: { kodeCab: [root.$store.state.auth.setCabang], ac: 'N', tgla: new Date().toLocaleString('sv', { timeZoneName: 'short' }).slice(0, 8) + '01', tglb: new Date().toLocaleString('sv', { timeZoneName: 'short' }).slice(0, 10), jnsCust: [] },
      tgl: {
    ...
    const transaksi = computed(() => {
      ...
      switch (dt.perPartner) {
        case true:
          a = dt.allCat ? data : data.filter(s => dt.filt.jnsCust.some(d => d === s.kategoriCust))
          break
        default:
          a = dt.allCat ? data.filter(s => dt.caraBayar.some(c => c === s.ct)) : data.filter(s => dt.filt.jnsCust.some(d => d === s.kategoriCust) && dt.caraBayar.some(c => c === s.ct))
      }
      return a
    })
    ...
    const jdl = computed(() => {
      ...
      if (props.jnsTrx === 'B') {
        ...
        x.push(
          ...
          { name: 'tglTempo', label: 'Tgl Tempo', field: row => row.tglTempo, align: 'left' },
          { name: 'status', label: 'Status', field: row => row.status, align: 'left', sortable: true },
          { name: 'note', label: 'Note', field: row => row.note, align: 'left' }
        )
      } else {
        x.push(
          { name: 'namaSales', label: 'Nama Sales', field: row => row.namaSales, sortable: true },
          { name: 'namaAkunBayar', label: 'Akun Jurnal', field: row => row.namaAkunBayar, align: 'left', style: 'white-space: normal !important', headerStyle: 'width: 400px !important' },
          { name: 'nomorPO', label: 'Nomor Pesanan', field: row => row.nomorPO, sortable: true },
          { name: 'tglTempo', label: 'Tgl Tempo', field: row => row.tglTempo, align: 'left' },
          ...
          .then(({ data }) => {
            ...
            dt.rkpTrx = aa.map(a => {
              return {
                ...
                kategoriCust: data.find(s => s.namaPartner === a).kategoriCust,
                totalHarga: data.filter(s => s.namaPartner === a).reduce((c, b) => root.$dwn.jumlah([c, b.totalHarga]), 0),
                totalAkhir: data.filter(s => s.namaPartner === a).reduce((c, b) => root.$dwn.jumlah([c, b.totalAkhir]), 0),
                totalPoint: data.filter(s => s.namaPartner === a).reduce((c, b) => root.$dwn.jumlah([c, b.totalPoint]), 0)
              }
            })
            dt.dataTrx = data
          })
    ...
    const tompo = (x) => {
      ...
      if ((x.ancab === 'Y' && x.asal === root.$store.state.auth.user.kodeCab && x.status === 'D') || (x.ancab === 'Y' && x.tujuan === root.$store.state.auth.user.kodeCab && x.status === 'W')) {
      ...
      } else if (props.jnsTrx === 'J' && (['tempo', 'Bank', 'EDC', 'eMoney'].includes(x.ct) && !['MAN', 'purchase', 'mitra', 'acc'].includes(root.$store.state.auth.user.userType)) && x.ancab !== 'Y' && x.ecommerce !== 'Y') {
        ...
        root.$q.dialog({
          ...
          if (x.status === 'D' && data.some(s => s === 'B')) {
            ...
            x.status = x.ancab === 'Y' && x.asal === root.$store.state.auth.user.kodeCab && data.some(s => s === 'T') ? 'D' : data.some(s => s === 'B') ? 'B' : 'T'
        ...
        root.$q.notify({ message: 'Harus diterima admin cabang...', color: 'warning' })
      }
    }
    ...
    const dataLamp = (x) => {
      root.$axios.get('/buktiTransaksi?', { params: { key: x.nomorBukti }})
        .then(({ data }) => {
          dt.lampTrans = data
        })
    }
    const downLamp = (x) => {
      root.$axios.get('/lampiranTransaksi', { params: { key: x }})
        .then(async (res) => {
    ...
    return { ...toRefs(dt), getLamp, dataLamp, downLamp, cekLog, cabPilih, repairJurnalServis, updPoint, ubahSales, buka, tompo, ubahTrx, ubahTgl, cabAsi, jdl, jenis, totalQty, totalAll, opac, totalHarga, getTrx, getdetTrx, getJur, onReset, onpil, rekapTransaksi, transaksi, jenisCust }
  },
  ...
  methods: {
    ...
    terima (x) {
      if (this.pr.ac === 'Y' && this.pr.asal === this.$store.state.auth.setCabang && x === 'T') {
    ...
    async printA4 (x) {
            ...
            <tr>
              <td colspan="4" class="">
                <table width='100%' class="bdr_top">
                  ...
                  <tr>
                    ${this.jnsTrx === 'J' ? '<td width="15%"><b>Sales person:</b><br/>' + x.namaSales + `</td><td><b>Payment:</b><br/>${x.ct}</td> ` : ''}
                  </tr>
                </table>
              </td>
            </tr>
      ...
      detail.data.forEach((det, i) => {
        dtDet += `<tr>
          ...
          <td align="right" style='text-align: right'>${new Intl.NumberFormat('en').format(Number(det.dpp / det.jmlKemasan).toFixed(2))}</td>
      ...
      let note = x.jnsTrx === 'J' && x.ancab !== 'Y' ? `
                      <div style='border : 1px dotted #000000; padding:10px' width='60%'>
                        ...
                        ${['tempo', 'Bank'].includes(x.ct) && x.jnsTrx === 'J' ? cabA.noteNota : ''}
    ...
    async printSjln (x, z) {
      ...
      let detail = await detTrx(x)
      let tqty = detail.data.reduce((a, b) => this.$dwn.jumlah([a, b.jmlKemasan]), 0)
      let cabA = this.jnsTrx === 'J' && x.asal === x.cabID ? this.cabAll.find(a => a.kodeCab === x.cabID) : this.cabAll.find(a => a.kodeCab === x.tujuan)
      ...
      detail.data.forEach((det, i) => {
        ...
        let snn = ''
        if (det.SN.length && z) {
          snn = '<ol>Serial No.' + det.SN.map((s, i) => {
            return `<li>${s}</li>`
          }).toString() + '</ol>'
        }
        dtDet += `<tr>
          <td>${(i + 1).toString().padStart(3, ' ')}. ${det.namaBarang} ${snn.replace(/\,/gi, '')}</td>
          <td style="text-align: right">${new Intl.NumberFormat('en').format(Number(det.jmlKemasan).toFixed(2))} ${det.kemasan.kemasan}</td>
        </tr>`
      })
      let ttp = `</table>
              </td>
    ...
    async printW (x) {
      ...
      detail.data.forEach((det, i) => {
        dtDet += `<tr>
            ...
            <td align='right'>${new Intl.NumberFormat('en').format(Number(det.jmlKemasan).toFixed(2))} ${det.kemasan.kemasan}</td>
            <td align='right'>${new Intl.NumberFormat('en').format(Number(det.dpp / det.jmlKemasan).toFixed(2))}</td>
